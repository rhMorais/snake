window.onload = function () {
    var body = document.body, //HTML
        tec = 39, tecAnterior = 39, pause,       //TECLAS
        limite = 25, inicio = 1, i, j, velocidade, interval, pontuacao = 0,//CAMPO
        posCorpo, posRabo, cabeca, corpo, rabo, cobra = [],//COBRA
    ...
    function iniciaCobra() {
        i = 1, j = 4;
        cabeca = document.querySelector("table tbody tr:nth-child(" + 1 + ") td:nth-child(" + 4 + ")");
        cobra.push(cabeca); //0
        corpo = document.querySelector("table tbody tr:nth-child(" + 1 + ") td:nth-child(" + 3 + ")");
        cobra.push(corpo); //1
        ...
        for (var b = (cobra.length - 1); b >= 0; b--) {
            if (b == 0) {
                ...
                if (!cobra[b].condicao || cobra[b].bunda) {
                    ...
                    alert("You dead LOSER!!");
                    cobra[b] = cobra[b+1];
                    cobra[b].className = "vermelho";
                    cobra[cobra.length - 1].className = "cinza";
                    return; 
                }
            }
            else if (b == (cobra.length - 1)) {
    ...
    // var caixaScore = document.createElement("fieldset id = 'score'");
    // document.querySelector("#score").appendChild(caixaScore);
    // caixaScore.appendChild(do)    
